...
const validateRoles = (allowedRoles: UserRoles[], userRole: UserRoles) => {
  ...
  if (!isAllowed)
    throw new HttpError(403, "Forbidden access to this service for this user");
};
...
export const middleware = async (
  ...
  if (token || validateToken) {
    ...
    try {
      ...
      return res
        .status(401)
        .send({ error: "Error authenticating user ".concat(error) });
    }
    if (roles) {
  ...
  if (queryValidation) {
    ...
    if (error)
      return res.status(400).send({
        error: "Request query params Validation Error\n".concat(error.message),
      });
  }
  // validate paths
  if (pathsValidation) {
    ...
    if (error)
      return res.status(400).send({
        error: "Request path params Validation Error\n".concat(error.message),
      });
  }
